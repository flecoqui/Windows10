...
//        private Windows.Storage.Streams.IOutputStream outputStream;
        public static SpeechToTextStream Create(uint duration = 0, uint threshold = 0)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine("Exception while creating CustomAudioStream: " + ex.Message);
            }
            return cas;
        }
        private SpeechToTextStream(uint d = 0,uint t = 0)
        {
            ...
            //outputStream = internalStream.GetOutputStreamAt(0);
...
//                                    byte[] updatedBuffer = new byte[4 + fmt.length + 8 + 8 + 8];
                                    //index += (ulong)buffer.Length;
        ...
        public Windows.Foundation.IAsyncOperation<bool> FlushAsync()
        {
            ...
            //return System.Runtime.InteropServices.WindowsRuntime.AsyncInfo.Run<bool>(_ => Task.Run(() => true));
        }
        ...
        public uint StoreWAVHeader(IBuffer buffer)
        {
            
            int length = 0;
            uint source = 0;
            if (IsTag(buffer.ToArray(source, 4), "RIFF") == true)
            {
                source += 4;
                if (GetInt(buffer.ToArray(source, 4), out length) == true)
                {
                    source += 4;
                    if (IsTag(buffer.ToArray(source, 4), "WAVE") == true)
                    {
                        source += 4;
                        Chunk c = new Chunk();
                        while ((source + 8 <=buffer.Length) && (ReadChunkHeader(buffer.ToArray(source, 8), c) == true))
                        {
                            source += 8;
                            if (IsTag(c.tag, "fmt ") == true)
                            {
                                ...
                                if ((source + c.length < buffer.Length) && (ReadChunkData(buffer.ToArray(source, (int)c.length), fmt) == true))
                                {
                                    uint nChannels = BitConverter.ToUInt16(fmt.data, 2);
                                    uint nSamplesPerSec = BitConverter.ToUInt32(fmt.data, 4);
                                    uint nAvgBytesPerSec = BitConverter.ToUInt32(fmt.data, 8);
                                    uint nBlockAlign = BitConverter.ToUInt16(fmt.data, 12);
                                    uint wBitsPerSample = BitConverter.ToUInt16(fmt.data, 14);

                                    if (duration > 0)
                                        indexMax = (duration * nAvgBytesPerSec) / 1000;
                                    else
                                        indexMax = 64000;
                                    source += c.length;
                                }
        ...
        public uint ParseWAVHeader(byte[] buffer)
        {
        ...
        public Windows.Foundation.IAsyncOperationWithProgress<uint, uint> WriteAsync(IBuffer buffer)
        {
            ...
            return System.Runtime.InteropServices.WindowsRuntime.AsyncInfo.Run<uint, uint>((token, progress) =>
            {
                return Task.Run(() =>
                {
                    ...
                    if (index == 0)
                    {
                        uint sourceIndex = StoreWAVHeader(buffer);
                        if (sourceIndex >0)
                        {
                            if (data.length == 0)
                                data.length = (uint)indexMax;
                            Length = data.length + 4 + fmt.length + 8 + 8 + 8;
                            byte[] updatedBuffer = new byte[4 + fmt.length + 8 + 8 + 8];
                            index += (ulong)updatedBuffer.Length;
                            //index += (ulong)buffer.Length;
                            System.Text.UTF8Encoding.UTF8.GetBytes("RIFF").CopyTo(0, updatedBuffer.AsBuffer(), 0, 4);
                            BitConverter.GetBytes(4 + fmt.length + 8 + data.length + 8).CopyTo(0, updatedBuffer.AsBuffer(), 4, 4);
                            System.Text.UTF8Encoding.UTF8.GetBytes("WAVE").CopyTo(0, updatedBuffer.AsBuffer(), 8, 4);
                            ...
                            System.Text.UTF8Encoding.UTF8.GetBytes("data").CopyTo(0, updatedBuffer.AsBuffer(), 20 + fmt.length, 4);
                            BitConverter.GetBytes(data.length).CopyTo(0, updatedBuffer.AsBuffer(), 24 + fmt.length, 4);
                            //ofs.Write(System.Text.UTF8Encoding.UTF8.GetBytes("RIFF"), 0, 4);
                            //ofs.Write(BitConverter.GetBytes(4 + fmt.length + 8 + c.length + 8), 0, 4);
                            //ofs.Write(System.Text.UTF8Encoding.UTF8.GetBytes("WAVE"), 0, 4);
                            //ofs.Write(System.Text.UTF8Encoding.UTF8.GetBytes("fmt "), 0, 4);
                            //ofs.Write(BitConverter.GetBytes(fmt.length), 0, 4);
                            //ofs.Write(fmt.data, 0, fmt.length);
                            //ofs.Write(System.Text.UTF8Encoding.UTF8.GetBytes("data"), 0, 4);
                            //ofs.Write(BitConverter.GetBytes(c.length), 0, 4)
                            System.Diagnostics.Debug.WriteLine("WriteAsync: " + updatedBuffer.Length.ToString() + " at position: " + internalStream.Position);
                            //return internalStream.WriteAsync(updatedBuffer.AsBuffer());
                            outputStream.WriteAsync(updatedBuffer.AsBuffer()).AsTask().Wait();
//                            outputStream.WriteAsync(buffer).AsTask().Wait();
                            if ((buffer.Length - sourceIndex)>0)
                                outputStream.WriteAsync(buffer.ToArray(sourceIndex, (int) (buffer.Length - sourceIndex)).AsBuffer()).AsTask().Wait();
                           // outputStream.WriteAsync(buffer).AsTask().Wait();
                            progress.Report((uint)buffer.Length);
                            return (uint)buffer.Length;
                        }
                    ...
                    else if (index < Length)
                    {
                        ...
                        //                return internalStream.WriteAsync(buffer);
                        outputStream.WriteAsync(buffer.ToArray(0, (int)byteToWrite).AsBuffer()).AsTask().Wait();
                        progress.Report((uint)buffer.Length);
                        return (uint) buffer.Length;
                    }
                    */
            ...
            return System.Runtime.InteropServices.WindowsRuntime.AsyncInfo.Run<uint, uint>((token, progress) =>
            {
                return Task.Run(() =>
                {
                    System.Diagnostics.Debug.WriteLine("WriteAsync: " + buffer.Length.ToString() + " bytes - Stream Size: " + internalStream.Size + " Stream position: " + internalStream.Position);
                    //System.Diagnostics.Debug.WriteLine("WriteAsync: " + buffer.Length.ToString());
