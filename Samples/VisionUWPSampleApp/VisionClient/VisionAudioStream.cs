...
namespace VisionClient
{
    ...
    /// </info>
    public class VisionAudioStream : IRandomAccessStream
    {
        ...
        public static VisionAudioStream Create(
            uint Channels, 
            uint SamplesPerSec, 
            uint AvgBytesPerSec,
            uint BlockAlign,
        ...
        {
            VisionAudioStream stts = null;
            try
            {
                stts = new VisionAudioStream(Channels, SamplesPerSec, AvgBytesPerSec, BlockAlign, BitsPerSample, AudioStart);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine("Exception while creating VisionAudioStream: " + ex.Message);
            }
            return stts;
        }
        private VisionAudioStream(
            uint Channels,
            uint SamplesPerSec,
            uint AvgBytesPerSec,
            uint BlockAlign,
