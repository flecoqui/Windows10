...
namespace VisionClient
{
    ...
    public delegate void StreamAudioLevelEventHandler(VisionMainStream sender, double reading);

    public delegate void StreamBufferReadyEventHandler(VisionMainStream sender);
    /// <summary>
    /// class VisionMainStream
    /// </summary>
    /// <info>
    /// Class used to:
    /// - record the audio samples in StorageFile
    /// - forward the audio samples towards the Vision REST API
    ...
    /// </info>
    public class VisionMainStream : IRandomAccessStream
    {
        ...
        private VisionAudioStream audioStream;
        ...
        private ConcurrentQueue<VisionAudioStream> audioQueue;
        public static VisionMainStream Create(ulong maxSizeInBytes, uint thresholdDuration, uint thresholdLevel)
        {
            VisionMainStream stts = null;
            try
            {
                stts = new VisionMainStream(maxSizeInBytes, thresholdDuration, thresholdLevel);
            }
            catch(Exception ex)
            {
                System.Diagnostics.Debug.WriteLine("Exception while creating VisionMainStream: " + ex.Message);
            }
            return stts;
        }
        private VisionMainStream(ulong MaxSizeInBytes = 0, uint ThresholdDuration = 0,uint ThresholdLevel = 0)
        {
            ...
            audioQueue = new ConcurrentQueue<VisionAudioStream>();
        }
                                ...
                                if (audioStream == null)
                                {
                                    if (internalStream.Size > thresholdDurationInBytes)
                                    {
                                        ...
                                        if (level > thresholdLevel)
                                        {
                                            ...
                                            audioStream = VisionAudioStream.Create(nChannels, nSamplesPerSec, nAvgBytesPerSec, nBlockAlign, wBitsPerSample, thresholdStart);
        ...
        public VisionAudioStream GetAudioStream()
        {
            VisionAudioStream audioStream = null;
            if (audioQueue != null)
            {
